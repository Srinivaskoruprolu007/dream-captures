...
import {
  ...
  GenerateImageDescriptionsOutput,
} from "@/ai/flows/generate-image-descriptions";
...
export function ImageDescriptionGenerator() {
  ...
  const [result, setResult] = useState<GenerateImageDescriptionsOutput | null>(
    null
  );
  ...
  return (
    <Card className="w-full max-w-2xl mx-auto border border-border/50 shadow-lg hover:shadow-xl transition-shadow duration-300">
      ...
      <CardContent className="p-6 space-y-6">
        ...
        <div className="space-y-2">
          ...
          <Input
            ...
            className="bg-input mb-2  focus:border-primary file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-primary/10 file:text-primary hover:file:bg-primary/20 cursor-pointer"
            aria-describedby="image-upload-hint"
          />
          <p id="image-upload-hint" className="text-xs text-muted-foreground">
            Upload an image (JPG, PNG, WEBP, max 4MB) to generate alt text and a
        ...
        >
          {isLoading ? (
            <>
              <span
                className="animate-spin inline-block h-4 w-4 border-2 border-current border-t-transparent rounded-full mr-2"
                role="status"
                aria-hidden="true"
              ></span>
            ...
            <>
              <Sparkles size={18} className="mr-2" /> Generate Descriptions
              (వివరణలను రూపొందించు)
            </>
          )}
        </Button>
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              <Label className="flex items-center gap-2 text-muted-foreground">
                <FileText className="h-4 w-4" /> Alt Text
              </Label>
              <Skeleton className="h-10 w-full" />
            </div>
            <div className="space-y-2">
              <Label className="flex items-center gap-2 text-muted-foreground">
                <MessageSquare className="h-4 w-4" /> Social Caption
              </Label>
        ...
        {result && !isLoading && (
          <div className="space-y-4 border-t border-border/50 pt-6 mt-6 animate-fade-in">
            ...
            <div className="space-y-2">
              <Label
                htmlFor="alt-text-output"
                className="flex items-center gap-2 text-foreground"
              >
                <FileText className="h-5 w-5 text-primary" /> Generated Alt Text
              </Label>
              <Textarea
                id="alt-text-output"
                readOnly
            ...
            <div className="space-y-2">
              <Label
                htmlFor="caption-output"
                className="flex items-center gap-2 text-foreground"
              >
                <MessageSquare className="h-5 w-5 text-primary" /> Generated
                Social Caption
              </Label>
              <Textarea
                id="caption-output"
      ...
      <CardFooter className="text-xs text-muted-foreground p-4 bg-muted/30 border-t border-border/30 text-center">
        AI generations can sometimes be inaccurate. Please review the content
        before using it.
      </CardFooter>
    </Card>
  );
