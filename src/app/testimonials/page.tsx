...
// Custom SVG Quote Mark - Use Secondary (Ice Blue)
const IndianQuoteMark = () => (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor" className="text-secondary opacity-70 mb-3 mx-auto" aria-hidden="true">
...
// Helper to render stars - Use Accent (Pink) for filled stars
const renderStars = (rating: number) => {
  const ratingLabel = `${rating} out of 5 stars`;

  return (
    <div className="flex items-center justify-center" role="img" aria-label={ratingLabel}>
      {Array.from({ length: 5 }, (_, i) => (
        <Star
          ...
          className={cn(
            ...
            i < rating ? 'text-accent fill-accent' : 'text-muted-foreground/30' // Use Accent Pink for filled stars
          )}
          aria-hidden="true"
        />
      ))}
...
export default function TestimonialsPage() {
  ...
  return (
    ...
    <div className="bg-gradient-to-b from-background via-primary/5 to-background"> {/* Mint tint gradient */}
      <div className="container mx-auto px-4 md:px-6 py-16 md:py-24">
        ...
        >
              ...
              <CarouselItem key={testimonial.id} className="pl-4 md:basis-1/2 lg:basis-1/3" role="group" aria-roledescription="slide" aria-label={`Testimonial from ${testimonial.name}`}>
                 <div className="p-1 h-full">
                    ...
                    <Card className="h-full flex flex-col justify-between border border-border/50 shadow-lg hover:shadow-xl transition-shadow duration-300 bg-card rounded-xl overflow-hidden transform hover:-translate-y-1">
                      <CardHeader className="flex flex-col items-center text-center pb-4 pt-6 bg-muted/30 border-b border-border/30 relative"> {/* Muted pastel header */}
                          {/* Use accent color for decorative line */}
                        ...
                        {/* Use primary color for avatar border */}
                        <Avatar className="h-20 w-20 mb-4 border-4 border-primary/50 shadow-md shrink-0">
                          <AvatarImage src={testimonial.avatar} alt="" data-ai-hint={testimonial.dataAiHint}/>
                          {/* Use secondary color for fallback */}
                          <AvatarFallback className="text-lg font-semibold bg-secondary/80 text-secondary-foreground">
                      ...
                      <CardContent className="flex-grow pt-6 px-6 pb-8 text-center">
                        ...
                        <blockquote className="text-muted-foreground italic leading-relaxed font-telugu">
                           <p lang={/^[؀-ۿऀ-ॿ](?:[؀-ۿऀ-ॿ\s:"!,.?()-]+)?$/.test(testimonial.quote) ? 'te' : undefined}>
                            "{testimonial.quote}"
                          </p>
                        </blockquote>
                      </CardContent>
          ...
          {/* Styled Previous/Next Buttons - Use pastel background */}
         ...
         <p className="text-center text-muted-foreground mt-16">
          ...
          Loved our service? We'd be thrilled to hear from you! <Link href="/contact" className="text-primary hover:text-accent font-semibold focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring rounded">Share Your Story</Link>.
        </p>
      </div>
    </div>
  );
