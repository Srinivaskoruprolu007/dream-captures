...
import { Camera, Heart, Users, Award, MapPin, Rings, Flower, Sparkles, Bell } from 'lucide-react'; // Added minimalist icons Rings, Flower, Sparkles, Bell
...
// Example Service Icons using Lucide
const WeddingIcon = () => <Rings aria-hidden="true" />;
const PartyIcon = () => <Users aria-hidden="true" />;
const ModelIcon = () => <Camera aria-hidden="true" />;
const OutdoorIcon = () => <Flower aria-hidden="true" />;


export default function Home() {
  return (
    <div className="flex flex-col min-h-dvh bg-background"> {/* Use background color */}
      {/* Hero Section - Pastel Overlay */}
      <section className="relative h-[70vh] md:h-[85vh] flex items-center justify-center text-center text-foreground overflow-hidden">
        {/* Background Image */}
        <Image
          src="https://picsum.photos/seed/telugu-hero/1920/1080"
          alt="Vibrant Telugu wedding ceremony scene"
          layout="fill"
          objectFit="cover"
          quality={85}
          className="absolute inset-0 z-0 filter brightness-90" // Slightly brighter
          priority
          data-ai-hint="wedding ceremony couple tradition"
          aria-hidden="true"
        />
        {/* Pastel Overlay (Mint Green) */}
        <div className="absolute inset-0 bg-gradient-to-t from-primary/30 via-primary/10 to-transparent z-10"></div>
        {/* Hero Content */}
        <div className="relative z-20 p-4 animate-fade-in-slow max-w-4xl mx-auto">
          <h1 className="text-4xl md:text-6xl lg:text-7xl font-serif font-bold mb-4 drop-shadow-md text-foreground"> {/* Darker text */}
            Dream Captures
          </h1>
           <p className="text-2xl md:text-3xl font-baloo text-accent mb-8 drop-shadow" lang="te"> {/* Accent Pink */}
             ప్రతి ఫోటోలో ఒక అందమైన గుర్తు...
          </p>
          <p className="text-lg md:text-xl mb-8 max-w-2xl mx-auto drop-shadow text-foreground/90"> {/* Darker text */}
          ...
          <div className="flex flex-col sm:flex-row justify-center items-center gap-4">
            ...
            <Button asChild size="lg" variant="default" className="font-semibold px-8 py-3 rounded-full shadow-lg transition-transform duration-300 hover:scale-105 w-full sm:w-auto">
              <Link href="/portfolio">View Our Work</Link>
            </Button>
            {/* Outline button (Secondary - Ice Blue) */}
             <Button asChild size="lg" variant="outline" className="border-secondary text-secondary hover:bg-secondary/10 hover:text-secondary-foreground font-semibold px-8 py-3 rounded-full shadow-lg transition-transform duration-300 hover:scale-105 w-full sm:w-auto">
      ...
      <section className="py-16 md:py-24 bg-background" aria-labelledby="about-teaser-heading">
        <div className="container mx-auto px-4 md:px-6 grid md:grid-cols-2 gap-12 items-center">
          {/* Image Container - Use pastel accent border */}
          <div className="relative aspect-[4/3] max-w-md mx-auto md:mx-0 rounded-lg overflow-hidden shadow-xl border-4 border-accent/30 order-1 md:order-none"> {/* Powder Pink border */}
             <Image
            ...
            />
             <div className="absolute inset-0 bg-gradient-to-t from-black/10 via-transparent" aria-hidden="true"></div> {/* Softer overlay */}
          </div>
          {/* Text Content */}
          <div className="animate-fade-in-slow order-2 md:order-none">
            <h2 id="about-teaser-heading" className="text-3xl md:text-4xl font-serif font-semibold mb-4 text-foreground">Meet the Team</h2>
             ...
             <div className="flex items-center text-muted-foreground text-sm mb-6">
                <MapPin size={16} className="mr-2 text-primary" /> Based in [Your City], Andhra Pradesh {/* Mint icon */}
            </div>
            {/* Outline button (Secondary - Ice Blue) */}
      ...
      {/* Portfolio Teaser - Light pastel background (e.g., Lavender tint) */}
       <section className="py-16 md:py-24 bg-lavender/10" aria-labelledby="portfolio-teaser-heading"> {/* Lavender tint background */}
        <div className="container mx-auto px-4 md:px-6 text-center">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-10">
           ...
           {/* Default button (Mint Green) */}
           <Button asChild variant="default" className="bg-primary hover:opacity-90 text-primary-foreground">
      ...
      {/* Testimonials Teaser - Use background color */}
       <section className="py-16 md:py-24 bg-background" aria-labelledby="testimonials-teaser-heading">
        <div className="container mx-auto px-4 md:px-6 text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            {testimonials.map((testimonial) => (
              <Card key={testimonial.id} className="text-left shadow-lg border border-border/50 hover:shadow-xl transition-shadow bg-card rounded-xl overflow-hidden"> {/* Softer border */}
                 <CardHeader className="flex flex-row items-center gap-4 pb-2 bg-muted/50 p-4"> {/* Muted pastel bg */}
                   <Avatar className="h-14 w-14 border-2 border-primary shrink-0"> {/* Mint border */}
                    <AvatarImage src={testimonial.avatar} alt={`Avatar of ${testimonial.name}`} data-ai-hint={testimonial.dataAiHint} />
                    <AvatarFallback className="bg-primary/20 text-primary">{testimonial.name.charAt(0)}</AvatarFallback> {/* Mint fallback */}
                  </Avatar>
                   <div>
                 ...
                 <CardContent className="pt-4 px-6 pb-6">
                   {/* Decorative Quote SVG - Accent Pink */}
                   <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" className="text-accent mb-2 opacity-70" aria-hidden="true">
                   ...
                   <blockquote className="text-muted-foreground italic font-noto" lang={testimonial.id === 3 ? 'te' : undefined}>
       ...
       {/* Services Teaser - Light pastel background (e.g., Ice Blue tint) */}
      <section className="py-16 md:py-24 bg-secondary/10" aria-labelledby="services-teaser-heading"> {/* Ice Blue tint */}
        <div className="container mx-auto px-4 md:px-6 text-center">
          <h2 id="services-teaser-heading" className="text-3xl md:text-4xl font-serif font-semibold mb-4 text-foreground">Our Signature Services</h2>
           ...
           {/* Responsive Grid for Services - Pastel cards */}
           <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 max-w-6xl mx-auto mb-10">
             {/* Service Card 1: Wedding - Primary (Mint) Theme */}
             <Card className="text-center shadow-lg hover:shadow-xl transition-shadow border border-border/50 rounded-xl overflow-hidden transform hover:-translate-y-1 flex flex-col bg-card">
               <CardHeader className="bg-primary/10 p-6"> {/* Mint tint bg */}
                  <div className="mx-auto h-16 w-16 flex items-center justify-center rounded-full bg-primary text-primary-foreground mb-4 shadow-md shrink-0">
                    <WeddingIcon />
                  </div>
              ...
              {/* Service Card 2: Parties - Secondary (Ice Blue) Theme */}
              <Card className="text-center shadow-lg hover:shadow-xl transition-shadow border border-border/50 rounded-xl overflow-hidden transform hover:-translate-y-1 flex flex-col bg-card">
                <CardHeader className="bg-secondary/10 p-6"> {/* Ice Blue tint bg */}
                 <div className="mx-auto h-16 w-16 flex items-center justify-center rounded-full bg-secondary text-secondary-foreground mb-4 shadow-md shrink-0">
                    <PartyIcon />
                 </div>
                 <CardTitle className="font-serif text-xl text-foreground">Parties & Events</CardTitle>
              ...
              {/* Service Card 3: Model Shoots - Accent (Pink) Theme */}
              <Card className="text-center shadow-lg hover:shadow-xl transition-shadow border border-border/50 rounded-xl overflow-hidden transform hover:-translate-y-1 flex flex-col bg-card">
                <CardHeader className="bg-accent/10 p-6"> {/* Pink tint bg */}
                 <div className="mx-auto h-16 w-16 flex items-center justify-center rounded-full bg-accent text-accent-foreground mb-4 shadow-md shrink-0">
                    <ModelIcon />
                 </div>
                 <CardTitle className="font-serif text-xl text-foreground">Model Shoots</CardTitle>
                  <CardDescription className="text-sm text-accent font-semibold font-noto" lang="te">మోడల్ షూట్స్</CardDescription>
               </CardHeader>
               <CardContent className="p-6 pt-4 flex-grow">
                 <p className="text-muted-foreground text-sm">Portfolio creation, fashion clicks, and creative portraits.</p>
               </CardContent>
             </Card>
             {/* Service Card 4: Outdoor Sessions - Lavender Theme */}
              <Card className="text-center shadow-lg hover:shadow-xl transition-shadow border border-border/50 rounded-xl overflow-hidden transform hover:-translate-y-1 flex flex-col bg-card">
                <CardHeader className="bg-lavender/10 p-6"> {/* Lavender tint bg */}
                 <div className="mx-auto h-16 w-16 flex items-center justify-center rounded-full bg-lavender text-lavender-foreground mb-4 shadow-md shrink-0">
                    <OutdoorIcon />
                 </div>
                 <CardTitle className="font-serif text-xl text-foreground">Outdoor Sessions</CardTitle>
                  <CardDescription className="text-sm text-lavender font-semibold font-noto" lang="te">అవుట్‌డోర్ ఫోటోలు</CardDescription>
               </CardHeader>
               <CardContent className="p-6 pt-4 flex-grow">
          ...
          {/* Secondary button (Ice Blue) */}
          <Button asChild variant="secondary" className="hover:opacity-90 text-secondary-foreground">
       ...
       <section className="py-16 md:py-24 bg-background text-center" aria-labelledby="final-cta-heading">
         <div className="container mx-auto px-4 md:px-6">
            <Award className="h-12 w-12 mx-auto text-accent mb-4" aria-hidden="true" /> {/* Accent Pink */}
            ...
            {/* Primary Button (Mint Green) */}
             <Button asChild size="lg" variant="default" className="rounded-full px-10 py-3 shadow-lg transition-transform duration-300 hover:scale-105">
